...
class DL():
    ...
    def what_is(self): # What is it?
        clear()
        print("Deep learning é um ramo da inteligência artificial que usa algoritmos complexos para treinar máquinas a agir como pessoas")
        self.back()

    def help_people(self): # Que maneiras este ramo ajudou
        clear()
        self.arr = ['Examinar os dados dos vídeos das câmaras de segurança e reconhecer quem são as pessoas', 'Previsão de padrões de falhas ou seja saber quando irá acontecer um erro em sua produção é um grande aliado quanto a competitividade do mercado']
